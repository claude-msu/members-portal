...
export default function Week3Lecture2() {
    ...
    return (
        <LectureLayout>
            ...
            <LectureP>
                A <strong className="text-foreground">tree</strong> is a connected acyclic graph; we'll implement one in the Week 8 activity (phonebook BST). Traversal: in-order (left, root, right) gives sorted order in a BST; pre-order and post-order for expression trees or structure. A <strong className="text-foreground">graph</strong> has nodes and edges; represent with adjacency list (<code className="text-xs bg-muted px-1.5 py-0.5 rounded border">{"vector<vector<int>>"}</code> or map of neighbors). <strong className="text-foreground">BFS</strong> (queue) gives shortest path in unweighted graphs and level order; <strong className="text-foreground">DFS</strong> (stack or recursion) for cycle detection, topological sort, exploring connected components. You'll practice these in the Week 8 activity and in Week 7 (DSA).
            </LectureP>
