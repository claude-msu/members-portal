-- Update the checkin_user_for_event function to remove active column check
-- and update RLS policies accordingly

-- Update RLS policy for event_qr_codes to not check for active = true
...
-- Update the checkin_user_for_event function to remove active = true check
...
BEGIN
  ...
  -- Get QR code details (removed active = true check)
...
-- Grant execute permission (in case it was lost)
