-- Fix the checkin_user_for_event function to remove expires_at check
-- since the expires_at column was dropped from event_qr_codes table
...
BEGIN
  ...
  -- Note: expires_at check removed since column was dropped
  -- QR codes are now valid until manually replaced
  ...
  VALUES (v_qr_code.event_id, v_user_id);
